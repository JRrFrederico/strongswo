...
window.BIBLE_VERSION                   = 'acf';                                                              // Define o identificador da versão
window.NOME_VERSAO_COMPLETA_BIBLIA     = 'Almeida Corrigida Fiel';                                           // Nome completo da versão
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções específicas para ACF.`);       // Loga o carregamento do script
...
// Este bloco carrega e exibe um versículo específico da Bíblia ACF.
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    console.log(`[ACF] Carregando: ${livro} ${capitulo}:${versiculo}`);                                      // Loga o carregamento do versículo
    ...
    if (!content) {                                                                                          // Verifica se o conteiner principal existe
        console.error("[ACF] Elemento .conteudo não encontrado.");                                           // Loga erro se não encontrar o conteiner
    ...
    try {
        const response = await fetch(`../versao/acf/${livro}/${capitulo}.json`);                             // Busca o arquivo JSON do capítulo
        if (!response.ok) {                                                                                  // Verifica se a requisição HTTP foi bem-sucedida
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (ACF)`);       // Lança um erro se a resposta não for 'ok'
        }
        ...
        console.error(`[ACF] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo} (ACF):`, error);   // Loga erro
        const textoP = document.createElement('p');                                                          // Cria elemento <p> para mensagem de erro
...
window.getSpecificChapterTitle = async function(livro, capitulo, versiculo) {
    console.log(`[ACF] Obtendo título interno para: ${livro} ${capitulo}:${versiculo}`);                     // Loga a busca do título
    try {                                                                                                    // Inicia um bloco try-catch para lidar com erros
        ...
        console.error(`[ACF] Erro ao obter título interno para ${livro} ${capitulo}:${versiculo} (ACF):`, error);// Loga erro
