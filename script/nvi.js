...
window.BIBLE_VERSION                   = 'nvi';                                                              // Define o identificador da versão
window.NOME_VERSAO_COMPLETA_BIBLIA     = 'Nova Versão Internacional';                                        // Nome completo da versão
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções específicas para NVI.`);       // Loga o carregamento do script
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    try {
        ...
        if (!response.ok) {                                                                                  // Verifica se a requisição HTTP foi bem-sucedida
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (NVI)`);       // Lança um erro se a resposta não for 'ok'
        }
        ...
        if (data.versiculos && data.versiculos[versiculo]) {                                                 // Verifica se o versículo existe nos dados carregados
            ...
            versiculoElementDiv.appendChild(textoP);                                                         // Adiciona o <p> à div do versículo
            console.warn(`[NVI] Versículo ${versiculo} não encontrado nos dados de ${livro} ${capitulo}.json (NVI)`);
        }
    } catch (error) {                                                                                        // Captura erros que possam ocorrer no bloco try
        console.error(`[NVI] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo} (NVI):`, error);   // Loga erro
        const textoP = document.createElement('p');                                                          // Cria elemento <p> para mensagem de erro
    ...
    if (window.titulo) {                                                                                     // Verifica se o elemento do título principal existe
        ...
        if (typeof window.getLivroDisplayName === 'function') {                                              // Verifica se a função para obter o nome formatado do livro existe
            ...
            console.warn("[NVI] Função window.getLivroDisplayName não encontrada. Usando chave do livro em maiúsculas para o título.");
        }
        ...
        console.warn(`[NVI] Elemento H2 principal (window.titulo) não encontrado para atualizar.`);
    }
};
...
window.getSpecificChapterTitle = async function(livro, capitulo, versiculo) {
    console.log(`[NVI] Obtendo título interno para: ${livro} ${capitulo}:${versiculo}`);                     // Loga a busca do título
