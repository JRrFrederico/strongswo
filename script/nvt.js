...
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções específicas para NVT.`);       // Loga o carregamento do script
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    try {
        ...
        if (!response.ok) {                                                                                  // Verifica se a requisição HTTP foi bem-sucedida
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (NVT)`);       // Lança um erro se a resposta não for 'ok'
        }
        ...
        if (data.versiculos && data.versiculos[versiculo]) {                                                 // Verifica se o versículo existe nos dados carregados
            ...
            versiculoElementDiv.appendChild(textoP);                                                         // Adiciona o <p> à div do versículo
            console.warn(`[NVT] Versículo ${versiculo} não encontrado nos dados de ${livro} ${capitulo}.json (NVT)`);
        }
    } catch (error) {                                                                                        // Captura erros que possam ocorrer no bloco try
        console.error(`[NVT] Erro ao carregar versículo ${livro} ${capitulo}:${versiculo} (NVT):`, error);   // Loga erro
        const textoP = document.createElement('p');                                                          // Cria elemento <p> para mensagem de erro
    ...
    if (window.titulo) {                                                                                     // Verifica se o elemento do título principal existe
        ...
        if (typeof window.getLivroDisplayName === 'function') {                                              // Verifica se a função para obter o nome formatado do livro existe
            ...
            console.warn("[NVT] Função window.getLivroDisplayName não encontrada. Usando chave do livro em maiúsculas para o título.");
        }
        ...
        console.warn(`[NVT] Elemento H2 principal (window.titulo) não encontrado para atualizar.`);
    }
};
...
window.getSpecificChapterTitle = async function(livro, capitulo, versiculo) {
    console.log(`[NVT] Obtendo título interno para: ${livro} ${capitulo}:${versiculo}`);                     // Loga a busca do título
    try {                                                                                                    // Inicia um bloco try-catch para lidar com erros
        const response = await fetch(`../versao/nvt/${livro}/${capitulo}.json`);                             // Busca o arquivo JSON do capítulo
        if (!response.ok) {                                                                                  // Verifica se a requisição foi bem-sucedida
            throw new Error(`HTTP ${response.status} ao buscar JSON para ${livro} ${capitulo} (NVT)`);
        }
