...
window.BIBLE_VERSION = 'original';                                                                           // Define o identificador da versão
window.NOME_VERSAO_COMPLETA_BIBLIA = 'Texto Original';                                                       // Nome completo da versão
console.log(`[${window.BIBLE_VERSION}.js] Script carregado. Definindo funções específicas para ORIGINAL.`);  // Loga o carregamento do script

//  Cria a função que será chamada por biblia-navegacao.js. Contagem dos dados específicos dos versículos (Original)
window.getSpecificVerseCount = function(livro, capitulo) {
    const versiculosPorCapitulo = {                                                                          // Objeto de contagem de versículos por capítulo para cada livro
        ...
        "ezequiel": { 1: 28, 2: 10, 3: 27, 4: 17, 5: 17, 6: 14, 7: 27, 8: 18, 9: 11, 10: 22, 11: 26, 12: 28, 13: 23, 14: 23, 15: 8, 16: 63, 17: 24, 18: 32, 19: 14, 20: 49, 21: 32, 22: 31, 23: 49, 24: 27, 25: 17, 26: 21, 27: 36, 28: 26, 29: 15, 30: 26, 31: 17, 32: 32, 33: 33, 34: 31, 35: 17, 36: 38, 37: 24, 38: 23, 39: 29, 40: 49, 41: 34, 42: 17 },
    ...
    // Este bloco busca a contagem para o livro e capítulo informados
    const contagem = versiculosPorCapitulo[livro]?.[capitulo];
    if (typeof contagem === 'undefined') {
        console.warn(`[ORIGINAL] Contagem não encontrada para ${livro} ${capitulo}`);          // Loga se não encontrar
        return 0;
    }
...
window.loadSpecificVerse = async function(livro, capitulo, versiculo) {
    ...
    // O elemento alvo para a renderização deve ser o contêiner de versículos já existente.
    const verseContentArea = document.querySelector('.conteudo-versiculos');
    if (!verseContentArea) {
        console.error("[ORIGINAL] Elemento '.conteudo-versiculos' não encontrado. Falha crítica na renderização.");
        return;
    }

    // Limpa o conteúdo anterior dentro de .conteudo-versiculos para evitar duplicatas.
    // Isso remove tanto o `div.texto-versiculo` quanto o `div.versiculo-ativo` genérico.
    verseContentArea.innerHTML = ''; 
    console.log("[ORIGINAL] Conteúdo de '.conteudo-versiculos' limpo.");
    ...
    verseContentArea.appendChild(versiculoElementDiv); // Anexa o elemento do versículo ao contêiner correto.
    console.log("[ORIGINAL] Div principal do versículo (.texto-versiculo) adicionada ao elemento '.conteudo-versiculos'.");

    // Atualiza o título principal do capítulo/versículo na área de conteúdo
    const tituloH2Element = document.querySelector('section.conteudo h2'); // Busca o H2 na section.conteudo
